...
@section('content')
    ...
    <form action="{{ route('content.update', $content->id) }}" method="post" enctype="multipart/form-data">
        ...
        <textarea name="description" id="description" cols="30" rows="10"> {{ old('description', $content->description) }}</textarea>
        @error('description')
            <span style="color:red">{{ $message }}</span>
        @enderror <br><br>

        <label for="duration">Duration:</label>
        <input type="number" name="duration" id="duration" value="{{ old('duration', $content->duration) }}">
        @error('duration')
            <span style="color:red">{{ $message }}</span>
        @enderror <br><br>
        ...
        <input type="date" name="release_date" id="release_date"
            value="{{ old('release_date', $content->release_date) }}">
        @error('release_date')
            <span style="color:red">{{ $message }}</span>
        @enderror <br><br>
